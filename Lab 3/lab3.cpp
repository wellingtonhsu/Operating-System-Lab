      ...
      if (instrList[i].op == 1){
       ...
       if (!checkPresent(instrList[i].page)){
         if (frameTable.size() < numFrames){
           ...
           setMap(instrList[i].page, frameNo);
           revframeTable[frameNo] = instrList[i].page;
           if(OCommand){
             printf("%d: ZERO %8d\n",instructionNo, frameNo);
             printf("%d: MAP  %4d%4d\n", instructionNo, instrList[i].page, frameNo);
           }
           zero++;
           mapped++;
           ...
           frameNo = algo->getFrame(frameTable, pageBits, revframeTable);
           setMap(instrList[i].page, frameNo);
           resetPresent(revframeTable[frameNo]);
           if(OCommand){
             printf("%d: UNMAP%4d%4d\n",instructionNo, revframeTable[frameNo], frameNo);
           }
           resetReferenced(revframeTable[frameNo]);
           unmapped++;

           //Check modified bit of the previous page
           if(checkModified(revframeTable[frameNo])){
             if(OCommand){
               printf("%d: OUT  %4d%4d\n",instructionNo, revframeTable[frameNo], frameNo);
             }
             resetModified(revframeTable[frameNo]);
             setPagedOut(revframeTable[frameNo]);
            out++;
           }
